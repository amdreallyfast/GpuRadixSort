...
//#include "Shaders/ComputeHeaders/NumDataEntries.comp"
    ...
    _numDataEntries(0)
{
    ...
    //// Note: The parallel prefix scan has each work group work on their own data (work group 0 does the prefix sum items 0-1023, work group 1 from 1024-2047, etc.).  It relies of a binary tree, so the work group size must be a power of 2.  The data that is being scanned for a prefix sum though might not be a power of 2.  To deal with this, make sure that there is always a work group for the data to be in (hence the +1), and that data that is not part of the data set is set to 0.
    //unsigned int numWorkGroups = (numDataEntries / ITEMS_PER_WORK_GROUP) + 1;

    // Note: The parallel prefix scan has each work group work on their own data (work group 0 does 
    // the prefix sum items 0-1023, work group 1 from 1024-2047, etc.).  It relies of a binary tree, 
    // so the work group size must be a power of 2.  The data that is being scanned for a prefix sum 
    // though might not be a power of 2.  To deal with this, make sure that there is always a work 
    // group for the data to be in (hence the +1), and that entries that are not part of the data 
    // set are set to 0.
...
void PrefixSumSsbo::ConfigureCompute(unsigned int computeProgramId, const std::string &bufferNameInShader)
{
    ...
    // TODO: remove ConfigureCompute and adapt the other SSBOs to dedicated binding locations (still need ConfigureRender(...) though)
...
{
    // TODO: make this a trivial base class virtual method instead a pure virtual
